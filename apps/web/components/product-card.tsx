...
export function ProductCard({ product, locale }: ProductCardProps) {
  ...
  const [localRating, setLocalRating] = useState(product.avgRating);
  ...
  const avgRating = localRating ? Number(localRating) : null;
            ...
            {avgRating ? (
              <>
                ...
                <span className="text-xs text-muted-foreground">
                  {avgRating.toFixed(1)} · {localCount} {t('reviews')}
                </span>
              </>
            ) : (
              <span className="text-xs text-muted-foreground">
                {localCount > 0 ? `${localCount} ${t('reviews')}` : (isBn ? 'এখনো রিভিউ নেই' : 'No reviews yet')}
              </span>
            )}
          </div>
        </div>

        {!reviewSuccess && !showReviewForm && isAuthenticated && (
          <button
